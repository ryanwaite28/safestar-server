...
import { PhoneVerifications } from '../models/user.model';
...
export async function create_phone_verification(
  ...
  const new_model = await PhoneVerifications.create(params);
  return new_model;
}
...
export async function query_phone_verification(
  ...
  try {
    const model_updates = await PhoneVerifications.findOne({ where: whereClause });
...
export async function delete_phone_verification(
  ...
  try {
    const model_updates = await PhoneVerifications.destroy({ where: whereClause });
    return model_updates;
  } catch (e) {
    console.log({
...
export async function update_phone_verification(
  ...
  try {
    const user_model_update = await PhoneVerifications.update(
      newState,
      { where: whereClause }
    );
